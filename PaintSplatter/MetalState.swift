...
public class metalState {
    ...
    public static var sharedDevice: MTLDevice?
    ...
    private var copy_pipeline: MTLRenderPipelineState?
    private var clamp_pipeline: MTLRenderPipelineState?
    private var alpha_pipeline: MTLRenderPipelineState?
    private var draw_pipeline: MTLRenderPipelineState?
    ...
    public func compileShader(named: String) -> MTLFunction {
        let shader = try ensure(String(contentsOf: #fileLiteral(resourceName: "Shaders.metal")))
        let library = try ensure(device?.makeLibrary(source: shader, options: nil))
    ...
    public func createComputePipeline(function: MTLFunction) -> MTLComputePipelineState {
        return try ensure(device?.makeComputePipelineState(function: function))
    }
    ...
    public func createRenderPipeline(vertex: MTLFunction, fragment: MTLFunction) -> MTLRenderPipelineState {
        ...
        return try ensure(device?.makeRenderPipelineState(descriptor: pipelineDescriptor))
    }
    ...
    public func combine(blurred: MTLTexture, weight w1: Float, noise: MTLTexture, weight w2: Float, color: Color, onto: MTLTexture? = nil) -> MTLTexture {
        ...
        var c_weight2 = w2
        let intermediate = CIColor.convert(color: color)
        var color = float3(Float(intermediate.red), Float(intermediate.green), Float(intermediate.blue))
